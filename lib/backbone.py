...
class MultiModalSwinTransformer(nn.Module):
    def __init__(self,
        ...
        self.conv = nn.Conv2d(num_features[-1], 768, 1, padding=0)
        self.self = Block(v_dim=768, l_dim=768, hidden_dim=768, num_heads=12, mlp_ratio=mlp_ratio, drop_path=0.1)
    ...
    def forward(self, x, l_feats, l_mask):
        ...
        for i_stage in range(self.num_layers):
            layer = self.layers[i_stage]
            x_res, x, Wh, Ww = layer(x, l_feats, l_mask, Wh, Ww)
            
            if i_stage == 3:
                norm_layer = getattr(self, f'norm{i_stage}')
                x_res = norm_layer(x_res)
                x_res = x_res.view(-1, Wh, Ww, self.num_features[-1]).permute(0, 3, 1, 2).contiguous()
                x_res = self.conv(x_res)
                x_res = x_res.flatten(2).transpose(1,2)
                x_res = self.self(x_res, x_res, None, Wh, Ww)
          
            else:
                norm_layer = getattr(self, f'norm{i_stage}')
                x_res = norm_layer(x_res)  # output of a Block has shape (B, H*W, dim)
                x_res = x_res.view(-1, Wh*2, Ww*2, self.num_features[i_stage]).permute(0, 3, 1, 2).contiguous()    
            outs.append(x_res)
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.fusion = PWAM(dim, dim, 768, dim, dim, int(dim//64))
        # patch merging layer
        ...
        if i_stage !=3:
            ...
            self.res_gate = nn.Sequential(nn.Linear(dim, dim, bias=False),
                nn.ReLU(),
    ...
    def forward(self, x, l_feats, l_mask, H, W):
        """ Forward function.
        ...
        if self.downsample is not None:
            ...
            return x_residual, x_down, Wh, Ww 
        else:
            return x_residual, x_residual, H, W
...
class PWAM(nn.Module):
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=8, dropout=0.0):
        ...
        self.vis_project = nn.Sequential(nn.Conv1d(dim, value_channels, 1, 1),  # the init function sets bias to 0 if bias is True
                                         nn.GELU(),
                                         nn.Dropout(dropout)
                                        )
        ...
        self.project_mm = nn.Sequential(nn.Conv1d(value_channels, dim, 1, 1),
                                        nn.GELU(),
                                        nn.Dropout(dropout)
                                        )
...
class SpatialImageLanguageAttention(nn.Module):
    def __init__(self, v_in_channels, l_in_channels, key_channels, value_channels, out_channels=None, num_heads=8):
