...
from ._utils import  METRIS

__all__ = ['metris']

def _segm_metris(pretrained, args):
    ...
    backbone = MultiModalSwinTransformer(embed_dim=embed_dim, depths=depths, num_heads=num_heads,
                                         ...
                                         ape=False, drop_path_rate=0.2, patch_norm=True,
    ...
    model = METRIS(backbone, args)
    return model


def _load_model_metris(pretrained, args):
    model = _segm_metris(pretrained, args)
    return model


def metris(pretrained='', args=None):
    return _load_model_metris(pretrained, args)
